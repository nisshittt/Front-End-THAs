...
const Card = (props) => {
  ...
  const deleteHandler = () => {
    // console.log(id)
    let newState = state.filter((element) => element.id !== id);
    setState(newState);
  };
  ...
  const doneHandler = () => {
    const array = state.map((data) => {
      if (data.id === id) {
        return {
          ...data,
          food: editData.foodName,
          cals: editData.calorieAmount,
        };
      } else {
        return data;
      }
    ...
    <div className="container">
      <div className="card">
        ...
        <div className="button">
          {isEditMode ? (
            <button id="edit" onClick={doneHandler}>
              Done
            </button>
          ) : (
